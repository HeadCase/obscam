...
export type InteractionProtection = "hover" | "focus" | "press" | "drag";

export interface OperatorUiState {
  chromeVisible: boolean;
  settingsOpen: boolean;
  lastActivityAtMs: number;
  protections: readonly InteractionProtection[];
  frameArrivalUntilMs: number | null;
  settingRejectionUntilMs: number | null;
}

export interface ExposureRailState {
  captureStartedAtUnixUs: number | null;
  exposureMs: number | null;
  progress: number | null;
}

export interface ExposureRailSample {
  exposureMs: number;
  startedAtUnixUs: number;
  nowUnixUs: number;
}

export type SettingPresentationPhase =
  | "unknown"
  | "visible"
  | "draft"
  | "requested"
...
export interface SettingPresentationFacts {
  visibleKnown: boolean;
  drafted: boolean;
  requestedDiffers: boolean;
  rejected: boolean;
  applied: boolean;
  awaitingPresentation: boolean;
}

export type OperatorUiEvent =
  | { type: "activity"; nowMs: number }
  | { type: "chrome_toggled"; nowMs: number }
  | { type: "tick"; nowMs: number }
  | {
      type: "protection_changed";
      protection: InteractionProtection;
      active: boolean;
      nowMs: number;
    }
  | { type: "settings_toggled"; nowMs: number }
  | { type: "settings_closed"; nowMs: number }
  | { type: "authority_granted"; nowMs: number }
  | { type: "authority_lost"; nowMs: number }
  | { type: "exact_frame_presented"; nowMs: number }
  | { type: "setting_rejected"; nowMs: number };

export function initialOperatorUiState(nowMs: number): OperatorUiState {
  return {
    chromeVisible: true,
    settingsOpen: false,
...
export function reduceOperatorUi(
  state: OperatorUiState,
  event: OperatorUiEvent
): OperatorUiState {
  switch (event.type) {
    case "activity":
    ...
    case "tick": {
      const frameArrivalUntilMs =
        state.frameArrivalUntilMs !== null && event.nowMs >= state.frameArrivalUntilMs
          ? null
          : state.frameArrivalUntilMs;
      const settingRejectionUntilMs =
        state.settingRejectionUntilMs !== null && event.nowMs >= state.settingRejectionUntilMs
          ? null
    ...
    case "protection_changed": {
      ...
      if (event.active) protections.add(event.protection);
      else protections.delete(event.protection);
      return {
        ...reveal(state, event.nowMs),
...
export function exposureRailProgress(
  exposureMs: number,
  startedAtMs: number,
  nowMs: number
): number | null {
  if (exposureMs < 1_000) return null;
  return Math.min(1, Math.max(0, (nowMs - startedAtMs) / exposureMs));
}

export function initialExposureRailState(): ExposureRailState {
  return {
...
export function advanceExposureRail(
  state: ExposureRailState,
  sample: ExposureRailSample | null
): ExposureRailState {
  if (sample === null || sample.exposureMs < 1_000) return initialExposureRailState();
  const progress = exposureRailProgress(
    sample.exposureMs,
    sample.startedAtUnixUs / 1_000,
    sample.nowUnixUs / 1_000
  );
  if (progress === null) return initialExposureRailState();
  const sameCapture =
    state.captureStartedAtUnixUs === sample.startedAtUnixUs &&
...
export function settingPresentationPhase(
  facts: SettingPresentationFacts
): SettingPresentationPhase {
  if (facts.rejected) return "rejected";
  if (facts.drafted) return "draft";
  if (!facts.visibleKnown) {
    if (!facts.awaitingPresentation) return "unknown";
...
function reveal(state: OperatorUiState, nowMs: number): OperatorUiState {

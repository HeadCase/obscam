...
/** Complete browser-owned state; browser APIs remain effects around this value. */
export interface ViewerState {
  runtimeEpoch: string;
  lifecycle: LifecycleFacts | null;
  control: ControlState;
  presentation: PresentationState;
  mediaConnection: "connecting" | "connected" | "disconnected";
  mediaConnectionGeneration: number;
  lastMediaPresentedAtUnixUs: number | null;
  awaitingCurrentPresentation: boolean;
  visibility: "visible" | "hidden";
  trustworthyFrame: FrameMapping | null;
...
/** Every authoritative fact or browser observation accepted by the root reducer. */
export type ViewerEvent =
  ...
  | { type: "tick"; nowUnixUs: number }
  | { type: "visibility"; visibility: "visible" | "hidden" }
  | {
      type: "evidence";
...
export interface ViewerProjection {
  status: "Live" | "Waiting for first image" | "Reconnecting" | "Stale" | "Unavailable";
  detail: string;
  controlConnection: ControlState["connection"];
  sourceGeneration?: number;
  frameCompletedAtUnixUs?: number;
  frameAgeMs?: number;
...
export function initialViewerState(
  runtimeEpoch: string,
  storedCredentials: StoredCredentials | null = null
): ViewerState {
  return {
    runtimeEpoch,
    lifecycle: null,
...
export function reduceViewer(state: ViewerState, event: ViewerEvent): ViewerTransition {
  let next = state;
    ...
    case "lifecycle": {
      ...
      next = {
        ...
        presentation: changedEpoch
          ...
          : state.presentation,
        lastMediaPresentedAtUnixUs:
          changedEpoch || mediaRecovered
            ? null
    ...
    case "evidence": {
      const allowance = event.p99DeliveryUs === null
        ...
        : Math.min(
            ...
            Math.max(MIN_DELIVERY_ALLOWANCE_US, event.p99DeliveryUs)
          );
      next = {
        ...state,
...
export function acceptsMediaPresentation(
  state: ViewerState,
  mediaConnectionGeneration: number
): boolean {
...
export function viewerProjection(state: ViewerState): ViewerProjection {
...
export function parseLifecycleFacts(value: unknown): LifecycleFacts {
  if (
    !isRecord(value) ||
...
function knownFrame(state: ViewerState, frame: FrameMapping) {
  const visibleLatencyMs = state.trustworthyPresentedAtUnixUs === null
    ? null
    : Math.max(
        0,
        Math.floor(
          (state.trustworthyPresentedAtUnixUs - frame.exposureCompletedAtUnixUs) / 1_000
        )
      );
  return {
...
function knownDetail(state: ViewerState, frame: FrameMapping, prefix: string): string {
...
function frameAgeMs(state: ViewerState, frame: FrameMapping): number {
  return Math.max(0, Math.floor((state.nowUnixUs - frame.exposureCompletedAtUnixUs) / 1_000));
}

function captureDeadlinePassed(
  state: ViewerState,
  capture: NonNullable<LifecycleFacts["capture"]>
): boolean {
  return state.nowUnixUs - capture.startedAtUnixUs >
    capture.exposureMs * 1_000 + state.deliveryAllowanceUs;
}

function failedComponent(facts: LifecycleFacts): "capture" | "encoder" | "relay" | null {
  if (facts.components.capture.state === "unavailable") return "capture";
  if (facts.components.encoder.state === "unavailable") return "encoder";

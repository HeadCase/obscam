...
- `/etc/obscam/mediamtx.yml` is the installed host configuration. It matches
  ...
  `wg0` and LAN addresses. Its control API listens only on the namespace-private
  `169.254.218.2:9997` address so ObsCam can observe exact relay-path readiness.
...
private control API reports the `obscam` path ready. Stopping MediaMTX must make

...
fn values_between_curated_detents_are_rejected() {
    assert!(CameraSettings::new(10, 100, Treatment::Monochrome).is_err());
    assert!(CameraSettings::new(20, 100, Treatment::Monochrome).is_err());
...
fn newer_pending_tuple_supersedes_the_old_target_without_advancing_applied_state() {
    ...
    let second = controller
        .accept(CameraSettings::new(50, 400, Treatment::Monochrome).expect("second tuple"))
        .expect("camera ready");
...
fn newest_pending_target_remains_authoritative_while_an_older_target_applies() {
    ...
    let pending = controller
        .accept(CameraSettings::new(50, 350, Treatment::Colour).expect("pending"))
        .expect("camera ready");
...
fn recovery_rejects_new_intent_until_the_previous_tuple_is_restored() {
    ...
    assert!(
        controller
            .accept(CameraSettings::new(50, 100, Treatment::Monochrome).expect("tuple"))
            .is_err()
    );
}
...
fn accepting_settings_interrupts_the_active_deterministic_exposure() {
    ...
    camera
        .configure(Settings::new(50_000, 100).expect("camera settings"))
        .expect("configure");

...
#[derive(Clone, Copy, Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct PresentationReport {
    schema_version: u8,
    client_id: Uuid,
    runtime_epoch: Uuid,
    connection_generation: u64,
...
impl ServiceQualityState {
    ...
    pub(crate) fn record(
        ...
        self.validate_version_and_epoch(report.schema_version, report.runtime_epoch)?;
        if report.connection_generation == 0
            || report.presented_frames == 0
            || report.clock_uncertainty_us > MAX_CLOCK_UNCERTAINTY_US
            || report.presented_at_unix_us.abs_diff(server_now_unix_us)
                > MAX_PRESENTATION_CLOCK_SKEW_US.saturating_add(report.clock_uncertainty_us)
            || matches!(report.stream_epoch, Some(0))
        {
            return Err(ServiceQualityError::InvalidReport);
        }

        let mapping = matches!(report.correlation, ReportedCorrelation::Exact)
            .then(|| {
                report
                    .stream_epoch
                    .zip(report.rtp_timestamp)
            ...
            .flatten();
        let sample = PresentationSample::from_report(report, mapping);
        ...
        let client = store
            .clients
            .iter_mut()
            .find(|client| client.client_id == report.client_id)
            .ok_or(ServiceQualityError::UnknownClient)?;
        if client.connection_generation != report.connection_generation {
            return Err(ServiceQualityError::StaleConnection);
        }
        if let Some((generation, previous)) = client.last_presented_frames
            && generation == report.connection_generation
        {
            if report.presented_frames <= previous {
                return Err(ServiceQualityError::InvalidReport);
            }
            client.presentation_skips = client
                .presentation_skips
                .checked_add(report.presented_frames - previous - 1)
                .expect("presentation skips exhausted");
        }
        client.last_presented_frames =
            Some((report.connection_generation, report.presented_frames));
        client.last_activity = activity;
    ...
    fn from_report(report: PresentationReport, mapping: Option<CorrelationMapping>) -> Self {
        let Some(mapping) = mapping else {
            return Self {
                ...
                presented_at_unix_us: report.presented_at_unix_us,
            };
        };
        ...
        Self {
            ...
            clock_uncertainty_us: Some(report.clock_uncertainty_us),
            presented_at_unix_us: report.presented_at_unix_us,
        }
    }
}
...
fn aggregate(clients: &[&ClientEvidence]) -> AggregateReport {
    ...
    for client in clients {
        ...
        for sample in &client.samples {
            ...
            partitions
                .entry(PartitionKey::new(client.client_id, *sample))
                .or_default()
                .push(*sample);
        }
    }
...
impl PartitionKey {
    fn new(client_id: Uuid, sample: PresentationSample) -> Self {
        Self {
            client_id,
            ...
            treatment: sample.treatment.map(Treatment::as_str),
            width: sample.width,
            height: sample.height,
            visibility: match sample.visibility {
...
mod tests {
    ...
    fn exact_samples_are_server_validated_and_unknowns_have_no_invented_facts() {
        ...
        exact.stream_epoch = Some(stream);
        exact.rtp_timestamp = Some(55_000);
        exact.correlation = ReportedCorrelation::Exact;
        exact.presented_at_unix_us = 1_200_000;
        ...
        assert_eq!(samples[1].clock_uncertainty_us, None);
    }
    ...
    fn exact_partition_reports_nearest_rank_latency_and_unique_cadence() {
        ...
        for ordinal in 1..=100_u64 {
            ...
            exact.correlation = ReportedCorrelation::Exact;
            exact.stream_epoch = Some(stream);
            exact.rtp_timestamp = Some(rtp_timestamp);
            exact.presented_at_unix_us = presented_at_unix_us;
            state
    ...
    fn settings_and_treatment_changes_create_compatible_partitions() {
        ...
        assert_eq!(partitions[0].key.treatment, Some("monochrome"));
        assert_eq!(partitions[1].key.settings_generation, Some(8));
        assert_eq!(partitions[1].key.treatment, Some("colour"));
    }

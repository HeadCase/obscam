...
- **Remote access** - Logs saved to `~/obscam-logs/` for SSH access
...
```bash
# View live logs
tail -f ~/obscam-logs/obscam.log

# Monitor errors only
tail -f ~/obscam-logs/obscam-error.log

# Remote log access via SSH
ssh pi@your-observatory "tail -f ~/obscam-logs/obscam.log"

# Download logs for analysis
scp pi@your-observatory:~/obscam-logs/*.log ./local-logs/
```

### Log Files
- `~/obscam-logs/obscam.log` - Main application log (INFO+)
- `~/obscam-logs/obscam-error.log` - Error-only log for quick issue identification  
- `~/obscam-logs/obscam-debug.log` - Detailed debug log (debug mode only)
...
grep "usb_failure" ~/obscam-logs/obscam.log
...
grep "WebSocket" ~/obscam-logs/obscam.log
...
```bash
# View session events
grep "session_event" ~/obscam-logs/obscam.log
...
grep "Performance" ~/obscam-logs/obscam.log

# View resource usage
grep "memory\|cpu" ~/obscam-logs/obscam.log
```
